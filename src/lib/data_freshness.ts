 ...
 */
// 奈良県は朝(6/7/8/9時)と夜(22時)に計5回試すが、これらは「その日のデータを
// 取るための複数回の挑戦」であって別々の締切ではない。朝の枠を1つずつ締切として
// 数えると、日中に3〜4回スキップ扱いになり、まだ22時便が残っているうちに
// 警告が出てしまう。そのため「その日の最後の機会」である22時だけを締切とみなす。
const NARA_PREF_RUN_HOURS_JST = [22];
const NARA_PREF = '奈良県';
/**
 * 1日に朝4回＋夜1回の計5回試すため、1日分落ちた程度なら翌日に取り返せる。
 * 2営業日続けてデータが入らなかった時点で知らせる。
 */
