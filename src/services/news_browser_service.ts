...
export async function fetchNewsViaBrowser(): Promise<NewsItem[]> {
    ...
    try {
        ...
        await page.goto('https://www.constnews.com/', { waitUntil: 'domcontentloaded', timeout: 8000 });
        ...
        const constNewsItems = await page.evaluate(() => {
            ...
            document.querySelectorAll('a').forEach((el, i) => {
                ...
                const isNoise = /^(ホーム|一覧|検索|ログイン|購読|お問い合わせ|会社案内|広告)$/.test(text);
                if (list.length < 10 && text.length >= 8 && text.length <= 140 && !isNoise && href.startsWith('http') && (text.includes('奈良') || href.includes('nara'))) {
                    list.push({
        ...
        // 検索ではなくエリアトップを狙う
        await page.goto('https://www.kentsu.co.jp/area/nara.asp', { waitUntil: 'domcontentloaded', timeout: 8000 });
        await page.waitForTimeout(1000);
        ...
        const kentsuItems = await page.evaluate(() => {
            ...
            document.querySelectorAll('.newslist li, .list_news a, a[href*="/news/"]').forEach((el, i) => {
                ...
                const href = (el as HTMLAnchorElement).href;
                const isNoise = /^(ホーム|一覧|検索|ログイン|購読|お問い合わせ|会社案内|広告)$/.test(text);
                if (text.length >= 8 && text.length <= 140 && !isNoise && href.startsWith('http') && (href.includes('/news/') || href.includes('nara'))) {
